...
Routes in `app/` render React components using content assembled by
`app/lib/content.ts`. Local JSON exports provide source records; curated
overrides supply selected profile, publication, and news updates.

Vite and vinext compile the site for Cloudflare Workers. `worker/index.ts`
...
| --- | --- |
| Page layout and page-specific copy | Relevant `app/**/page.tsx` |
| Research, people, and intern source records | `content-source/pages.json` |
| Curated profiles and content parsing | `app/lib/content.ts` |
| Recent news | `latestNews` in `app/lib/content.ts` |
| Publications | Publication logic in `app/lib/content.ts` and records in `content-source/pages.json` |
| Images and documents | `public/media/` and `content-source/site-media-map.json` |
| Navigation | `app/components/Header.tsx` and `Footer.tsx` |
...
- `wrangler.deploy.jsonc`: the site origin, named `mohan-lab-website`.
